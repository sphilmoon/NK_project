...
output_dir <- "/home/outputs/totalNK_outputs/2_umap/wo_51_52/rds"
...
for (dim_name in names(integrated_data_list)) {
  ...
  for (res in res_list) {
    ...
    # Save the Seurat object for this dims and resolution
    rds_output_file <- file.path(output_dir, paste0("integrated_data_", dim_name, "_res", res, "genecounts.rds"))
    saveRDS(integrated_data, rds_output_file)
    cat("💾 Seurat object saved to", rds_output_file, "\n")
...
cat("✅ Cell counts per animal saved to", animal_counts, "\n")
