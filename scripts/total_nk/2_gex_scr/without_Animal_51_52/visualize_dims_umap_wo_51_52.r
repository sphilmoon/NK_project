...
#          for dims20 (resolution 0.4) and dims25 (resolution 0.3).
...
resolutions <- c(0.4, 0.3)  # dims20: 0.4, dims25: 0.3
names(resolutions) <- dim_names
...
# Visualize UMAP for each dimensionality
for (dim_name in names(integrated_data_list)) {
  ...
  ggsave(output_file,
         ...
         dpi = 600,
         units = "in")
  cat("✅ Combined UMAP plot for", dim_name, "saved to", output_file, "\n")
}
