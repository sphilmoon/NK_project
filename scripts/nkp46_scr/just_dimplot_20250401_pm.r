...
if (!"condition" %in% colnames(integrated_data@meta.data)) {
  ...
  # Debug: Check NCR1 expression distribution
  print("NCR1 expression summary:")
  print(summary(ncr1_exp))
...
# Debug: Check condition distribution before subsetting
print("Condition distribution before subsetting:")
print(table(integrated_data$condition))
...
# Debug: Check condition distribution after subsetting
print("Condition distribution after subsetting:")
print(table(integrated_data$condition))
...
# Step 7.5: Generate DimPlot to Visualize NKp46+ and NKp46- Conditions
cat("🌟 Generating DimPlot for NKp46+ and NKp46- conditions...\n")

# DimPlot colored by condition (nkp46+ vs nkp46-)
dim_plot_condition <- DimPlot(
  integrated_data,
  group.by = "condition",
  label = TRUE,
  pt.size = 0.5,
  cols = c("nkp46+" = "red", "nkp46-" = "blue")
) +
  ggtitle("UMAP: NKp46+ vs NKp46- Conditions") +
  theme(
    plot.title = element_text(hjust = 0.5),
    legend.position = "right"
  )

# DimPlot colored by animal_condition (animal + condition)
dim_plot_animal_condition <- DimPlot(
  integrated_data,
  group.by = "animal_condition",
  label = TRUE,
  pt.size = 0.5
) +
  ggtitle("UMAP: NKp46+ and NKp46- by Animal") +
  theme(
    plot.title = element_text(hjust = 0.5),
    legend.position = "right"
  ) +
...
# Save the plots
ggsave(
  filename = file.path(dge_output_dir, "dimplot_nkp46_conditions.pdf"),
  plot = dim_plot_condition,
  width = 10,
  height = 8,
  dpi = 600
)
ggsave(
  filename = file.path(dge_output_dir, "dimplot_nkp46_by_animal.pdf"),
  plot = dim_plot_animal_condition,
  width = 12,
  height = 8,
  dpi = 600
)

cat("📊 DimPlots saved to output directory.\n")
