...
# ------------------------- #
# UMAP with Numbered Clusters for Merged Object
# ------------------------- #
cat("🎨 Generating UMAP with numbered clusters for merged object...\n")
umap_clusters <- DimPlot(
    merged_obj,
    group.by = "seurat_clusters", # Use cluster numbers
    label = TRUE, # Add cluster numbers as labels
    label.size = 5, # Size of the cluster labels
    pt.size = 0.3, # Point size for readability
    repel = TRUE # Prevent label overlap
) +
    ggtitle("UMAP with Numbered Clusters (Merged Object)") +
    theme(plot.title = element_text(hjust = 0.5, size = 14))

ggsave(
    file.path(pdf_dir, "UMAP_clusters_merged_20250531.pdf"),
    umap_clusters,
    width = 10, height = 8, dpi = 600
)

cat("✅ UMAP with numbered clusters saved\n")
cat("✅ All UMAP visualizations complete\n")
